...
class PermissionsService
{
     ...
     */
    public function __construct( RoleService $roleService, UserService $userService, $anonymousUserID = 10 )
    {
    ...
    public function getCurrentUser()
    {
        ...
        {
            $this->currentUser = $this->userService->loadUser( $this->anonymousUserID );
        }
    ...
    public function hasAccess( $module, $function, User $user = null )
    {
        ...
        // @todo Merge in Interface changes that makes RoleAssigments part of API
        ...
        foreach ( $spiRoleAssignments as $spiRoleAssignment )
        {
            ...
            $role = $roleService->loadRole( $spiRoleAssignment->roleId );
            foreach ( $role->policies as $policy )
            {
                if ( $policy->module === '*' && $spiRoleAssignment->limitationIdentifier === null )
                    return true;

                if ( $policy->module !== $module && $policy->module !== '*' )
                    continue;

                if ( $policy->function === '*' && $spiRoleAssignment->limitationIdentifier === null )
                    return true;

                if ( $policy->function !== $function && $policy->function !== '*' )
                    continue;

                if ( $policy->limitations === '*' && $spiRoleAssignment->limitationIdentifier === null )
                    return true;
